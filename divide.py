...
def main():
    ...
    for i in range(len(obj['annotations'])):
        ...
        if 'num_keypoints' in obj['annotations'][i].keys():
            image_name = obj['images'][i]['url'].split('/')[-1].split('.')[0]
            ...
            # pose json파일 저장
            with open('pose_json/{}.json'.format(i), 'w', encoding='utf-8') as make_file:
    ...
    all_bbox = {
        "swing{}_detect.json".format(
            str(int(json_file.split('_')[-1].split('.')[0])).zfill(4)): all_bbox
    }
